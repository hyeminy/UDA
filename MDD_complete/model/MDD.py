...
class MDD(object):
    ...
    def get_loss(self, inputs, labels_source):
        ...
        classifier_loss = class_criterion(
            outputs.narrow(0,0,labels_source.size(0)),
            labels_source
            )
        ...
        classifier_loss_adv_src = class_criterion(
            outputs_adv.narrow(0,0,labels_source.size(0)),
            target_adv_src
        )
...
class MDDNet(nn.Module):
    def __init__(self, base_net = 'resnet50', use_bottleneck=True, bottleneck_dim = 1024, width=1024, class_num=31):
        ...
        self.classifier_layer_list_2 = [
            nn.Linear(bottleneck_dim, width),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(width,class_num)
        ]
        self.classifier_layer_2 = nn.Sequential(*self.classifier_layer_list_2)
